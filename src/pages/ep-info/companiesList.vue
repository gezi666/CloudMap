        ...
        <div class="overflowAuto" v-loading="loading" element-loading-text="数据加载中...">
            <table class="court-table">
                ...
                <tr v-for="(list,index) in datalist">
                    ...
                    <td>{{list.moCategory | isValue}}</td>
                    <td>{{list.admAreaCode | isValue}}</td>
                    <td>{{list.province | isValue}}</td>
                    <td>{{list.legalPersonCode | isValue}}</td>
...
<script type="es6">
    ...
    export default {
        ...
        methods: {
            ...
            getList(params){//列表
                api.Get('/cloudMap/ep/getCompaniesList',params).then((res)=>{
                    this.loading=false;
